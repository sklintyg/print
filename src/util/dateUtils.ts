...
export const _dateReg =
  /[1-9][0-9]{3}-(0[1-9]|1[0-2])-(0[1-9]|[1-2][0-9]|3[0-1])/;
export const _dateRegDashesOptional =
  /[1-9][0-9]{3}-?(0[1-9]|1[0-2])-?(0[1-9]|[1-2][0-9]|3[0-1])/;
...
export const epochDaysAdjustedToTimezone = (date: Date): number =>
  Math.floor((date.getTime() - date.getTimezoneOffset() * 60000) / 8.64e7);
...
export const getValidDate = (
  ...
  if (dateValue) {
    if (
      !isBefore(dateValue, _minAllowedDate) &&
      !isAfter(dateValue, _maxAllowedDate)
    )
      return dateValue;
  }
};

export const isValidUncertainDate = (dateString: string): boolean =>
  /[0-9]{4}-[0-9]{2}-[0-9]{2}/.test(dateString);

export const isValidDateIncludingSpecialDateCodes = (
  dateString: string | undefined
) => {
  if (!dateString) return;

  return (
    isValid(getValidDate(dateString)) ||
    dayCodeReg.test(dateString) ||
    weekCodeReg.test(dateString) ||
    monthCodeReg.test(dateString)
  );
};
...
export const getLatestPeriodEndDate = (
  configList: ConfigUeCheckboxDateRange[],
  valueList: ValueDateRange[]
): Date | undefined => {
  let maxDate: Date | undefined;

  for (let i = 0; i < configList.length; i++) {
    const currPeriodId = configList[i].id;
    const currPeriodValue = valueList.find(
...
export const getPeriodHasOverlap = (
  valueList: ValueDateRange[],
  ...
  valueList.forEach((period) => {
    ...
    if (
      isSameDay(fromDate, currentFromDate) ||
      isSameDay(toDate, currentToDate)
    ) {
      overlapFound = true;
    } else if (
      ...
      areIntervalsOverlapping(
        ...
        { start: currentFromDate, end: currentToDate },
        { inclusive: true }
      )
    ) {
      overlapFound = true;
...
export const getNumberOfSickLeavePeriodDays = (
  periods: ValueDateRange[]
): number => {
  let total = 0;

  for (let i = 0; i < periods.length; i++) {
    if (getValidDate(periods[i].from) && getValidDate(periods[i].to)) {
      total += getPeriodWorkDays(
        getValidDate(periods[i].from)!,
        getValidDate(periods[i].to)!
      );
    }
  }
...
export const filterDateRangeValueList = (
  valueList: ValueDateRange[]
): ValueDateRange[] => {
  return valueList
    .map((val) => ({
...
export const isDateRangeValidOrIncomplete = (
  fromDateInput: string,
  toDateInput: string
): boolean => {
  const areBothDatesValid =
    isValid(getValidDate(fromDateInput)) && isValid(getValidDate(toDateInput));
  return (
    !areBothDatesValid ||
    !isBefore(getValidDate(toDateInput)!, getValidDate(fromDateInput)!)
  );
