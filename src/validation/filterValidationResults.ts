...
} from "../types/certificateValidation.types";
import { ValidationResult } from "./getValidationResults";

function resolvePriorityBetweenValidationTypes(
  result: ValidationResult,
  _: number,
  list: ValidationResult[]
) {
  if (
    result.validation.type === CertificateDataValidationType.SHOW_VALIDATION
  ) {
    return !list.some(
      (validation) =>
  ...
  if (
    result.validation.type === CertificateDataValidationType.HIDE_VALIDATION
  ) {
    return !list.some(
      (validation) =>
        validation.validation.type ===
...
const shouldValidationRuleBeMerged = (
...
const getAffectedIds = (validation: CertificateDataValidation) =>
  typeof validation.id === "string" ? [validation.id] : validation.id;
...
const filterSameRule =
  ...
  (result: ValidationResult): boolean => {
    return !(
      ...
      (result.validation.id === undefined ||
        isEqual(
          getAffectedIds(result.validation),
          getAffectedIds(needle.validation)
        ))
    );
  };
...
export const filterValidationResults = (
  ...
  validationResults
    .filter(resolvePriorityBetweenValidationTypes)
    .reduce((result: ValidationResult[], current: ValidationResult) => {
