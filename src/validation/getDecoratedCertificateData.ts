...
function shouldBeDisabled(metadata: MetaData, links: ResourceLink[]) {
  return (
...
function getDisabledSubElements(
  ...
  const value =
    ...
    config.type === ConfigTypes.UE_CHECKBOX_MULTIPLE_CODE
      ? {
          ...element.value,
      ...
      switch (validation.type) {
        ...
        case CertificateDataValidationType.HIGHLIGHT_VALIDATION:
          return {
            ...el,
            style: result
              ? CertificateDataElementStyleEnum.HIGHLIGHTED
              : CertificateDataElementStyleEnum.NORMAL,
          };
  ...
  return shouldBeReadOnly(metadata)
    ...
    : Object.fromEntries(
        Object.entries(data).map(([id, element]) => [
          id,
          validateElement(data, element),
        ])
      );
}

export function getDecoratedCertificateData(
  data: CertificateData,
  metadata: MetaData,
  links: ResourceLink[]
): CertificateData {
  ...
  return validateData(
    Object.fromEntries(
      Object.entries(data).map(([id, element]) =>
        readOnly
          ? [id, { ...element, visible: isVisible(element), readOnly }]
          : [id, { ...element, visible: isVisible(element), disabled }]
      )
    ),
    metadata
  );
