 ...
 */
export const differenceInDays = (a: Date | number, b: Date | number): number =>
  Math.floor(differenceInHours(a, b) / 24);
      ...
      switch (value.type) {
        ...
        case CertificateDataValueType.DATE_RANGE:
          return {
            ...result,
            [field]:
              isValid(getValidDate(value.from)) &&
              isValid(getValidDate(value.to)),
